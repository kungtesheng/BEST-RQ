...
from fairseq.models.speech_to_text.modules.convolution import (
    Conv1dSubsampler,
    Conv2dSubsampler,
    Conv2dSubsampler_without_padding_mask,
)
...
################################################################
        ...
        self.Subsampler = Conv2dSubsampler(
            input_channels,
            input_feat_per_channels,
            conv_out_channels,
            encoder_embed_dim,
        )
        self.Subsampler_without_padding_mask = Conv2dSubsampler_without_padding_mask(
            input_channels,
            input_feat_per_channels,
            conv_out_channels,
            encoder_embed_dim,
...
##############################################################################
        if padding_mask is not None:
            src_length = torch.sum(padding_mask, dim = 0)
            
            if self.feature_grad_mult > 0:
                feature, input_lengths = self.Subsampler(source, src_length)
                if self.feature_grad_mult != 1.0:
                    features = GradMultiply.apply(features, self.feature_grad_mult)
            else:
                with torch.no_grad():
                    feature, input_lengths = self.Subsampler(source, src_length)
            padding_mask = lengths_to_padding_mask(input_lengths)
        
        if padding_mask is None:
            if self.feature_grad_mult > 0:
                feature = self.Subsampler_without_padding_mask(source)
                if self.feature_grad_mult != 1.0:
                    features = GradMultiply.apply(features, self.feature_grad_mult)
            else:
                with torch.no_grad():
